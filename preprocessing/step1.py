...
def binarize_per_slice(image, spacing, intensity_th=-400, sigma=1, area_th=30, eccen_th=0.99, bg_patch_size=10):
...
def all_slice_analysis(bw, spacing, cut_num=0, vol_limit=[0.68, 7.5], area_th=5e3, dist_th=65):
...
if __name__ == '__main__':
    INPUT_PATH = '/media/pami4/yolayDATA/airwayCT/AirwaySegmentationGT-anon/case1/Image.nii'
    case_pixels, m1, m2, mtotal, origin, spacing = step1_python(INPUT_PATH)
    save_itk(mtotal.astype(dtype='uint8'),origin, spacing,'/media/pami4/yolayDATA/airwaySeg/prep_result/test.nii')
#    plt.imshow(m1[60])
#    plt.figure()
#    plt.imshow(m2[60])
